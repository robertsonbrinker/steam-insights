...
MIN_GAME_COUNT = 200#20  # Minimum number of distinct games with BOTH attributes
MIN_TOTAL_REVIEWS = 270000#2700  # Minimum total reviews across all games with BOTH attributes
...
def analyze_pair_type(pair_type, table1, table2, col1, col2, use_ordering=True):
    ...
    Args:
        pair_type: Name of the pair type (e.g., 'tag+tag')
        table1: First table name (e.g., 'tags')
        table2: Second table name (e.g., 'genres')
        col1: First column name (e.g., 'tag')
        col2: Second column name (e.g., 'genre')
        use_ordering: Whether to use < ordering to avoid duplicates (for same-table pairs)
    """

    ordering_clause = f"AND t1.{col1} < t2.{col2}" if use_ordering else ""

    query = base_query + f"""
-- Get pairs with their median Wilson scores
...
-- Calculate overall median for games without both attributes
pairs_with_inverse AS (
    SELECT
        ...
        pmw.total_reviews,
        (
            SELECT COUNT(DISTINCT gws.app_id)
            FROM game_wilson_scores gws
            WHERE NOT (
                EXISTS (SELECT 1 FROM {table1} t WHERE t.app_id = gws.app_id AND t.{col1} = pmw.attribute_1)
                AND EXISTS (SELECT 1 FROM {table2} t WHERE t.app_id = gws.app_id AND t.{col2} = pmw.attribute_2)
            )
        ) AS game_count_inverse,
        (
            SELECT MEDIAN(gws.wilson_score)
            FROM game_wilson_scores gws
            WHERE NOT (
                EXISTS (SELECT 1 FROM {table1} t WHERE t.app_id = gws.app_id AND t.{col1} = pmw.attribute_1)
                AND EXISTS (SELECT 1 FROM {table2} t WHERE t.app_id = gws.app_id AND t.{col2} = pmw.attribute_2)
            )
        ) AS median_wilson_score_inverse,
        (
            SELECT SUM(gws.total)
            FROM game_wilson_scores gws
            WHERE NOT (
                EXISTS (SELECT 1 FROM {table1} t WHERE t.app_id = gws.app_id AND t.{col1} = pmw.attribute_1)
                AND EXISTS (SELECT 1 FROM {table2} t WHERE t.app_id = gws.app_id AND t.{col2} = pmw.attribute_2)
            )
        ) AS total_reviews_inverse
...
if all_results:
    ...
    combined_results = combined_results.sort_values('wilson_diff', ascending=False).head(2000)

    print("\n" + "=" * 120)
    print("Top attribute pairs by Wilson score difference:")
    print(combined_results.to_string(index=False))

    # Save to CSV
    combined_results.to_csv('wilson_scores_by_attribute_pairs.csv', index=False)
    print("\n✓ Results saved to wilson_scores_by_attribute_pairs.csv")
else:
